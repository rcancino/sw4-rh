...
import com.luxsoft.sw4.rh.sat.*
  ...
  def importarBancos(){
	...
	sql.eachRow("select * from sw4_rh.sat_banco"){ row->
 ...
 def importarDeducciones(){
	def sql=new Sql(ctx.dataSource)
	sql.eachRow("select * from sw4_rh.sat_deduccion"){ row->
 ...
 def importarIncapacidades(){
	def sql=new Sql(ctx.dataSource)
	sql.eachRow("select * from sw4_rh.sat_incapacidad"){ row->
 ...
 def importarPercepciones(){
	def sql=new Sql(ctx.dataSource)
	sql.eachRow("select * from sw4_rh.sat_percepcion"){ row->
 ...
 def importarRegimenContratacion(){
	def sql=new Sql(ctx.dataSource)
	sql.eachRow("select * from sw4_rh.sat_regimen_contratacion"){ row->
 ...
 def importarRiesgoPuesto(){
	def sql=new Sql(ctx.dataSource)
	sql.eachRow("select * from sw4_rh.sat_riesgo_puesto"){ row->
